...
function calcAndCacheDimensions(carousels) {
  ...
  for (let carousel of carousels) {
    ...
    for (let i = 0; i < placeholders.length; i++) {
      ...
      imagesDimensionsArray.push(
        images[i].parentElement.getBoundingClientRect()
      );
    }
...
function animateSlides(direction, containerId) {
  ...
  for (let image of images) {
     ...
     */
    const isLastElementGoingRight =
      nextIndex === 0 && direction == Directions.NEXT;
    const isLastElementGoingLeft =
      nextIndex === n - 1 && direction == Directions.PREV;
    ...
    const imageRect =
      carouselsDimensionsCache[containerId].images[originalIndex];
    const nextPosRect =
      carouselsDimensionsCache[containerId].placeholders[nextIndex];
